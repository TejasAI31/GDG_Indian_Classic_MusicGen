  ...
  const getCursorStyle = (): React.CSSProperties => {
    return {
      background: `radial-gradient(circle 200px at ${mousePosition.x}px ${mousePosition.y}px, 
                  rgba(150, 150, 150, 0.3), transparent 70%)`,
      ...
      >
        <Link href="/explore" className="relative z-10" onClick={(e) => isRevealed ? null : e.preventDefault()}>
          <CardSection 
            ...
            delay={200} 
            emoji="🎧" 
            description="Immerse yourself in high-fidelity audio streaming"
            imageName="explore.png"
          />
        </Link>
        <Link href="/upload-analyze" className="relative z-10" onClick={(e) => isRevealed ? null : e.preventDefault()}>
          <CardSection 
            title="Analyze" 
            isRevealed={isRevealed} 
            delay={400} 
            emoji="🎶" 
            description="Find new artists tailored to your unique taste"
        ...
        <Link href="/instruments" className="relative z-10" onClick={(e) => isRevealed ? null : e.preventDefault()}>
          <CardSection 
            ...
            delay={600} 
            emoji="🎹" 
            description="Produce and share your own music with powerful tools"
...
const CardSection: React.FC<CardSectionProps> = ({ 
  ...
  return (
    ...
    >
      ...
      <Card className="h-96 overflow-hidden relative cursor-pointer group 
                       hover:shadow-xl border-0 transition-all duration-500 
                       ease-[cubic-bezier(0.4,0,0.2,1)] hover:shadow-gray-500/30">
        {/* Background image with dark overlay */}
        <div 
          className="absolute inset-0 bg-cover bg-center transition-all duration-700 group-hover:scale-105"
          style={{ backgroundImage: `url(${imagePath})` }}
        />
        <div className="absolute inset-0 bg-black/60 transition-all duration-700 group-hover:bg-black/50" />
        <div className="absolute inset-0 backdrop-blur-sm transition-opacity duration-700 opacity-30 group-hover:opacity-50" />
        ...
        <CardContent className="relative h-full flex flex-col items-center justify-center p-6 z-10">
          <div className="w-24 h-24 mb-6 rounded-full flex items-center justify-center 
                         bg-gray-800/80 border border-gray-600 transition-all duration-500 
                         ease-[cubic-bezier(0.4,0,0.2,1)] group-hover:scale-110 group-hover:bg-gray-800/60">
            <span className="text-4xl text-gray-100 transition-transform duration-500 
          ...
          <p className="text-center text-gray-300 transition-all duration-500 
